...
Admin.generateKey = async (type) => {
    try {
        ...
        const key = uuid().split('-').join('').slice(0, 20).toUpperCase().match(/.{1,5}/g).join('-')

        await knex('keys').insert({
            discord_id     : null,
            activation_key : key,
            expires_at     : moment().add(Admin.key_types[type], 'days').format('YYYY-MM-DD HH:mm:ss')
        })
...
Admin.getKeys = async () => {
    try {
        const keys = await knex('keys')
            .leftOuterJoin('discords', 'discords.id', '=', 'keys.discord_id')
